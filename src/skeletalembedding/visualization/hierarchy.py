...
def plot_hierarchy(model, *, show_ancestry=False, max_points=5000, figsize=None):
    ...
    axes = [figure.add_subplot(rows, columns, i + 1, projection="3d" if dimension == 3 else None)
    ...
    sample = np.linspace(0, len(original) - 1, min(max_points, len(original)), dtype=int)
    for index, (level, ax) in enumerate(zip(model.levels_, axes)):
        ...
        if show_ancestry and index > 0:
            ...
            take = np.linspace(0, len(fine.points) - 1, min(max_points, len(fine.points)), dtype=int)
            for child in take:
                ax.plot(*coordinates([fine.points[child], level.points[fine.parent_indices[child]]]),
                        color="0.6", alpha=0.3, linewidth=0.5)
        ...
        for region in topology.get("endpoints", []):
            ax.scatter(*coordinates([region.center]), marker="s", color="tab:green", s=30)
        suffix = " • selected" if index == model.selected_backbone_level_ else ""
        ax.set_title(f"Level {index}: {len(level.points)} representatives{suffix}\n"
    ...
    for route, kind in zip(model.routes_, model.element_types_):
        axes[-1].plot(*coordinates(route.samples), color="tab:orange" if kind == "rib" else "tab:blue")
