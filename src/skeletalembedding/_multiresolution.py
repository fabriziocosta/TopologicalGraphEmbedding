...
def validate_hierarchy_parameters(model):
    for name, minimum in (("hierarchy_max_levels", 0), ("hierarchy_target_size", 3),
                          ("hierarchy_local_neighbors", 2), ("backbone_max_representatives", 3),
    ...
    if level != "auto" and (isinstance(level, bool) or not isinstance(level, (int, np.integer)) or level < 0):
...
def medoid(points, members, original_indices, method="medoid", random_state=0):
    ...
    candidates = members
    if method == "approx_medoid" and len(members) > 64:
        candidates = np.random.default_rng(random_state).choice(members, 64, replace=False)
    ...
    for start in range(0, len(candidates), 128):
        ...
        for offset in range(0, len(members), 1024):
            costs += cdist(points[block], points[members[offset:offset + 1024]]).sum(axis=1)
...
def build_hierarchy(points, *, max_levels=8, target_size=1000, min_reduction=0.15,
                    distance_quantile=0.1, local_neighbors=10,
                    representative_method="medoid", random_state=0):
    points = np.asarray(points, dtype=float)
    levels = [RepresentativeLevel(points, np.arange(len(points)), None,
                                   [np.array([i], dtype=int) for i in range(len(points))], 0.0)]
    for depth in range(max_levels):
        ...
        floor = max(float(np.median(positive)) * 1e-12, np.finfo(float).tiny) if positive.size else 1.0
        scales = np.maximum(distances[:, -1], floor)
        normalized = distances[:, 1:] / np.sqrt(scales[:, None] * scales[neighbors[:, 1:]])
        threshold = float(np.quantile(normalized, distance_quantile))
        _, inverse, counts = np.unique(current.points, axis=0, return_inverse=True, return_counts=True)
        duplicates = {}
        if np.any(counts > 1):
        ...
        for seed in np.argsort(current.representative_indices, kind="stable"):
            if assigned[seed] >= 0:
                continue
            row = neighbors[seed]
            admissible = distances[seed] / np.sqrt(scales[seed] * scales[row]) <= threshold
            group = np.unique(np.r_[seed, row[admissible & (assigned[row] < 0)]])
            if duplicates:
                extra = [duplicates[int(label)] for label in np.unique(inverse[group]) if int(label) in duplicates]
                if extra:
        ...
        if len(groups) == n or reduction < min_reduction:
            break
        selected = np.array([medoid(current.points, group, current.representative_indices,
                                    representative_method, random_state + depth)
                             for group in groups])
        descendants = [np.sort(np.concatenate([current.descendant_indices[i] for i in group]))
                       for group in groups]
        current.parent_indices = assigned
        levels.append(RepresentativeLevel(current.points[selected], current.representative_indices[selected],
                                           None, descendants, float(np.quantile(scales, distance_quantile))))
    return levels
...
def initialize_hierarchy(model, points):
    ...
    model.hierarchy_levels_ = build_hierarchy(
        points, max_levels=model.hierarchy_max_levels if model.use_multiresolution else 0,
        target_size=model.hierarchy_target_size, min_reduction=model.hierarchy_min_reduction,
        distance_quantile=model.hierarchy_distance_quantile, local_neighbors=model.hierarchy_local_neighbors,
        representative_method=model.representative_method, random_state=model.random_state)
    ...
    model._resolution_probe_models_ = []
    model._structural_subsamples_ = []
    model.mip_candidate_count_ = 0
    model.hierarchy_summary_ = {"n_levels": len(model.levels_), "level_sizes": model.hierarchy_sizes_,
                              "selected_backbone_level": 0, "selection_reason": "single_level",
                              "stable_cycle_count_by_level": [], "stable_junction_count_by_level": []}
...
def evaluate_level(model, level, index):
    ...
    points = level.points
    diagram, backend = _estimate_persistence(points, max_points=model.persistence_max_points,
                                            random_state=model.random_state)
    scale = _local_scale(points)
    normalized = _normalize_persistence_diagram(diagram, scale)
    threshold = 3.0 if model.persistence_threshold is None else model.persistence_threshold
    bars = normalized[np.isfinite(normalized).all(axis=1) & (np.diff(normalized, axis=1).ravel() >= threshold)]
    if not model.detect_cycles:
        bars = bars[:0]
    bars = bars[np.argsort(-(bars[:, 1] - bars[:, 0]), kind="stable")][:model.max_cycles]
    prototypes = _kmeans(points, min(model.n_centroids, len(points)), model.random_state)
    probe = copy(model)
    probe.levels_ = [level]
    ...
    probe.persistent_cycles_ = [PersistentCycle(float(b), float(d), float(d-b)) for b, d in bars]
    original_metric = points * model.scale_ + model.mean_
    probe.linear_structure_ = _is_nearly_linear(prototypes * model.scale_ + model.mean_, model.linear_structure_tolerance)
    ...
    return {"status": "tested", "cycle_count": probe.requested_cycle_count_, "persistent_cycles": probe.persistent_cycles_,
                "junctions": probe.junction_regions_, "endpoints": probe.endpoint_regions_, "graph": graph, "scale": scale,
                "persistence_diagram": diagram, "normalized_persistence_diagram": normalized,
                "persistence_backend": backend, "level": index, "n_points": len(points)}
...
def infer_hierarchy_topology(model):
    ...
    eligible = [i for i, level in enumerate(model.levels_) if len(level.points) >= minimum]
    bounded = [i for i in eligible if len(model.levels_[i].points) <= model.backbone_max_representatives]
    chosen = set(bounded)
    if bounded:
    ...
    for i, level in enumerate(model.levels_):
        if i not in chosen:
            model.topology_by_level_[i] = {"status": "skipped", "reason": "size_or_neighborhood_limit"}
            continue
        try:
    ...
    tested = [i for i in sorted(chosen) if model.topology_by_level_[i]["status"] == "tested"]
    if not tested:
        raise ValueError("No hierarchy level has sufficient representatives for topology; increase hierarchy_target_size")
    bands = []
    for i in tested:
        band = [i]
        for j in range(i - 1, max(-1, i - model.backbone_consensus_levels), -1):
            ...
            matches = match_junctions_across_levels(a["junctions"], b["junctions"],
                                                    tolerance=4 * max(a["scale"], b["scale"]))
            if a["cycle_count"] != b["cycle_count"] or len(a["junctions"]) != len(b["junctions"]) or not np.all(matches):
                break
            band.append(j)
    ...
    bands = [band for band in bands if model.topology_by_level_[band[0]]["cycle_count"] == richest]
    guarded = []
    for band in bands:
        ...
        if finer:
            ...
            matches = match_junctions_across_levels(guard["junctions"], entry["junctions"],
    ...
    bands = guarded
    reason = "stable_band"
    within = [band for band in bands if len(model.levels_[band[0]].points) <= model.backbone_max_representatives]
    if model.backbone_level != "auto":
        if model.backbone_level not in tested:
            raise ValueError("The explicit backbone_level could not be evaluated")
        selected, band, reason = model.backbone_level, [model.backbone_level], "explicit"
    ...
    if len(model.levels_[selected].points) > model.backbone_max_representatives:
        warnings.warn("Preserving topology requires exceeding backbone_max_representatives.", RuntimeWarning, stacklevel=3)
        reason += ":size_budget_exceeded"
    model.hierarchy_summary_.update(selected_backbone_level=selected, selection_reason=reason,
        stable_cycle_count_by_level=[model.topology_by_level_[i].get("cycle_count") for i in range(len(model.levels_))],
        stable_junction_count_by_level=[len(model.topology_by_level_[i]["junctions"]) if i in tested else None
                                       for i in range(len(model.levels_))])


def sparse_corridor_path(points, start, end, neighbors=10, *, waypoints=None, model=None):
    ...
    distances, indices = cKDTree(points).query(points, k=min(neighbors + 1, len(points)))
    if distances.ndim == 1:
    ...
    if model is not None:
        ...
        costs = model.routing_length_weight * costs / max(float(np.median(scales)), 1e-12)
        costs += model.routing_density_weight * distances[:, 1:].ravel() / np.sqrt(scales[rows] * scales[columns])
        if model.use_local_pca and model.routing_tangent_weight:
            ...
            costs += model.routing_tangent_weight * (1 - np.abs(np.sum(tangents[rows] * tangents[columns], axis=1)))
    graph = csr_matrix((np.maximum(costs, 1e-12), (rows, columns)), shape=(len(points), len(points)))
...
def refine_backbone(model, graph, paths):
    ...
    model.coarse_backbone_paths_ = {key: model.routing_graph_.points[path.vertices].copy()
    ...
    for key, path in paths.items():
        ...
        descendant_ids = np.unique(np.concatenate([model.levels_[level_index].descendant_indices[v] for v in vertices]))
        ...
        for depth in range(level_index, 0, -1):
            ...
            positions = {int(original): i for i, original in enumerate(fine.representative_indices[fine_ids])}
            local_path = sparse_corridor_path(fine.points[fine_ids], positions[int(anchors[0])],
                                              positions[int(anchors[-1])], model.hierarchy_local_neighbors,
                                              waypoints=[positions[int(anchor)] for anchor in anchors], model=model)
            if len(local_path) < 2:
                near = cKDTree(fine.points).query(support, k=min(model.hierarchy_local_neighbors, len(fine.points)))[1]
                fine_ids = np.unique(np.r_[fine_ids, np.asarray(near).ravel()])
                positions = {int(original): i for i, original in enumerate(fine.representative_indices[fine_ids])}
                local_path = sparse_corridor_path(fine.points[fine_ids], positions[int(anchors[0])],
                                                  positions[int(anchors[-1])], 2 * model.hierarchy_local_neighbors,
                                                  waypoints=[positions[int(anchor)] for anchor in anchors], model=model)
            if len(local_path) < 2:
                model.route_refinement_failures_.append({"edge": key, "level": depth - 1})
                break
...
def finish_hierarchy_diagnostics(model):
    ...
    if not model.topology_by_level_:
        model.topology_by_level_[0] = {
            "status": "tested", "cycle_count": model.persistent_cycle_count_,
            "persistent_cycles": model.persistent_cycles_, "junctions": model.junction_regions_,
            "endpoints": model.endpoint_regions_, "scale": model.local_scale_,
            "n_points": model.backbone_input_size_, "graph": model.routing_graph_,
            "persistence_backend": model.persistence_backend_,
        }
        model.hierarchy_summary_.update(stable_cycle_count_by_level=[model.persistent_cycle_count_],
    ...
    model.topology_input_sizes_ = {i: entry["n_points"] for i, entry in model.topology_by_level_.items()
    ...
    if len(tested) > 1:
        model.cycle_resolution_support_ = np.mean([match_cycles_across_levels(
            cycles, entry["persistent_cycles"], tolerance=4 * max(model.local_scale_, entry["scale"])) for entry in tested], axis=0)
        model.junction_resolution_support_ = np.mean([match_junctions_across_levels(
            junctions, entry["junctions"], tolerance=4 * max(model.local_scale_, entry["scale"])) for entry in tested], axis=0)
    for cycle, support in zip(cycles, model.cycle_resolution_support_):
        cycle.resolution_support = float(support)
        cycle.subsample_support = float(cycle.stability_support) if model.stability_selection else float("nan")
    for junction, support in zip(junctions, model.junction_resolution_support_):
        ...
        locations = []
        for entry in tested:
            matches = [region for region in entry["junctions"]
                       if region.branch_count == junction.branch_count
                       and np.linalg.norm(region.center - junction.center) <= 4 * max(model.local_scale_, entry["scale"])]
            match = min(matches, key=lambda region: np.linalg.norm(region.center - junction.center)) if matches else None
            junction.branch_count_by_level[entry.get("level", 0)] = match.branch_count if match is not None else None
            if match is not None:
                locations.append(match.center)
        junction.location_dispersion = (float(np.sqrt(np.mean(np.sum((np.asarray(locations) - np.mean(locations, axis=0)) ** 2, axis=1))))
                                        if len(locations) > 1 else float("nan"))
    model.cycle_subsample_support_ = (np.asarray(model.cycle_support_).copy() if model.stability_selection
                                     else np.full(len(cycles), np.nan))
    count = getattr(model, "backbone_element_count_", len(model.routes_))
    model.route_resolution_support_ = np.array([path_resolution_support(model, route.samples)
                                                for route in model.routes_[:count]])
    model.rib_resolution_support_ = np.asarray(model.rib_resolution_support_, dtype=float)
    consensus = list(getattr(model, "junction_consensus_", []))
    if len(consensus) != len(junctions):
        consensus = [{"center": junction.center.copy(), "branch_count": junction.branch_count}
                     for junction in junctions]
    for record, junction in zip(consensus, junctions):
        record.update(resolution_support=junction.resolution_support,
    ...
    for route in model.routes_:
        ...
        model.route_descendant_support_.append({"n_original_descendants": len(descendants),
