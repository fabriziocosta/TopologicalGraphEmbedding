...
def test_hierarchy_partitions_and_parent_composition():
    ...
    for index, level in enumerate(levels):
        ...
        assert np.array_equal(np.sort(np.concatenate(level.descendant_indices)), np.arange(len(points)))
        assert level.descendant_original_indices is level.descendant_indices
        for original, descendants in zip(level.representative_indices, level.descendant_indices):
        ...
        if index + 1 < len(levels):
            ...
            for group, descendants in enumerate(parent.descendant_indices):
                ...
                assert np.array_equal(descendants, np.sort(np.concatenate([level.descendant_indices[i] for i in children])))
        else:
...
def test_medoid_exact_objective_and_approximate_determinism():
    ...
    assert medoid(points, members, members, 'approx_medoid', 2) == medoid(points, members, members, 'approx_medoid', 2)
    # Equal-cost candidates are resolved by original row, not iteration order.
    assert medoid(np.array([[0.], [1.]]), [1, 0], np.array([8, 3])) == 1
...
def test_duplicate_and_stopping_cases():
    ...
    assert len(build_hierarchy(points, target_size=3, min_reduction=.999)) == 1


@pytest.mark.parametrize('parameter,value', [
    ('hierarchy_max_levels', -1), ('hierarchy_target_size', 2),
    ('hierarchy_distance_quantile', 1), ('hierarchy_min_reduction', 0),
    ('hierarchy_local_neighbors', True), ('representative_method', 'centroid'),
    ('backbone_level', -1), ('backbone_consensus_levels', 0),
    ('route_resolution_weight', float('nan')), ('rib_resolution_weight', -1),
...
def test_spatial_cycle_and_degree_matching():
    ...
    assert not match_cycles_across_levels([left], [remote], tolerance=.5)[0]
    assert match_cycles_across_levels([left], [left], tolerance=.5)[0]
    junction = JunctionRegion(np.zeros(2), 3, 1)
    incompatible = JunctionRegion(np.zeros(2), 4, 1)
    compatible = JunctionRegion(np.ones(2)*.1, 3, 1)
    assert match_junctions_across_levels([junction], [incompatible, compatible], tolerance=.5)[0]


@pytest.mark.parametrize('name,cycles', [('circle', 1), ('figure-eight', 2), ('star', 0), ('loop-branch', 1)])
def test_multilevel_synthetic_topology(name, cycles):
    points = generate_synthetic_datasets(n=300, noise=.01, random_state=0)[name]
    ...
    assert model.coarse_backbone_graph_.cycle_rank() == model.backbone_graph_.cycle_rank()
    ...
    if name == 'circle':
        assert model.cycle_resolution_support_[0] > .8
        assert len(model.route_chains_[0]['points']) > model.backbone_input_size_
        assert np.mean(model.transform(points).residual_norm) < .1
    if name == 'star':
...
def test_disabled_single_level_and_refit():
    ...
    assert np.allclose(default.transform(points).projected, disabled.transform(points).projected)
...
def test_ancestry_covariance_retains_gaussian_weights(closed):
    ...
    blocks = [(block, min(local_t[block]), max(local_t[block])) for block in np.array_split(np.arange(100), 8)]
    for center in [0, .3, .99]:
        for bandwidth in [.002, .1]:
            distances = _route_distances(local_t, center, closed)
            weights = np.exp(-.5*((distances-min(distances))/bandwidth)**2)
            expected = (coordinates*weights[:, None]).T @ coordinates / weights.sum()
            actual = _ancestry_covariance(coordinates, local_t, blocks, center, closed, bandwidth)
...
def test_density_robustness_and_rare_branch():
    ...
    assert any(index >= len(circle) + len(dense) for index in levels[-1].representative_indices)
...
def test_sklearn_parameter_forwarding():
    ...
    for cls in [SkeletalEmbeddingClassifier, SkeletalEmbeddingTransformer]:
        model = clone(cls(hierarchy_target_size=30, representative_method='approx_medoid'))
...
def test_hierarchy_plot():
    ...
    model = SkeletalEmbedding(hierarchy_target_size=20).fit(np.column_stack([np.cos(t), np.sin(t)]))
...
def test_stable_band_fallback_and_explicit_level(monkeypatch):
    ...
    def fake_evaluate(model, level, index):
        ...
        return {'status': 'tested', 'cycle_count': count, 'junctions': [], 'endpoints': [],
                'persistent_cycles': [], 'scale': .1, 'n_points': len(level.points), 'level': index}
...
def test_no_dense_distances_in_compression(monkeypatch):
    ...
    monkeypatch.setattr(_topology, '_pairwise_distances', lambda *_: pytest.fail('global pairwise distances'))
...
def test_multiresolution_torus():
    ...
    u, v = u.ravel(), v.ravel()
    points = np.column_stack(((2+.8*np.cos(v))*np.cos(u), (2+.8*np.cos(v))*np.sin(u), .8*np.sin(v)))
    model = SkeletalEmbedding(hierarchy_target_size=80, n_centroids=12, standardize=False,
        persistence_max_points=180, persistence_threshold=1.5, max_cycles=2,
        max_residual_dim=1, residual_subspace_smoothness=.2, coverage_refinement=True,
        coverage_max_iterations=1, coverage_max_candidates_per_iteration=3,
        coverage_error_tolerance=.1).fit(points)
    assert model.backbone_input_size_ < len(points)
    assert sum(entry.get('cycle_count') == 2 for entry in model.topology_by_level_.values()) >= 2
    assert model.realized_cycle_count_ == 2
    assert np.all(model.cycle_resolution_support_ > .5)
...
def test_rib_sources_deduplication_and_resolution_utility():
    ...
    first = RibCandidate(points, 0, 0, 'transverse', 2., .8, resolution_support=.8)
    duplicate = RibCandidate(points.copy(), 1, 0, 'hierarchy', 1., .7,
                             resolution_support=.7, seed_sources=('hierarchy',))
    level = RepresentativeLevel(points, np.arange(3), None, [np.array([i]) for i in range(3)], .1)
    model = SimpleNamespace(levels_=[level, level], use_multiresolution=True, rib_seed_source='residual',
        _structural_subsamples_=[], local_scale_=.1, coverage_min_gain=0, rib_min_support=.6,
    ...
    other = RibCandidate(points+3, 2, 0, 'transverse', 2., .8, resolution_support=0)
    chosen = select_ribs([other, first], max_ribs=1, min_gain=0, length_penalty=0,
                        rib_penalty=0, junction_penalty=0, resolution_weight=1)
    assert chosen == [first]
...
def test_structural_subsampling_precedes_mip(monkeypatch):
    ...
    def record(candidates, *args, **kwargs):
        seen.append(any(np.isfinite(candidate.subsample_support) for candidate in candidates))
        return original_select(candidates, *args, **kwargs)

    monkeypatch.setattr(module, 'select_backbone_mip', record)
    t = np.linspace(0, 2*np.pi, 100, endpoint=False)
    model = SkeletalEmbedding(hierarchy_target_size=30, stability_selection=True,
                             stability_runs=2, n_centroids=12).fit(np.column_stack([np.cos(t), np.sin(t)]))
    assert seen[0]
