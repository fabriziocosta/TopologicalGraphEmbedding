"""Schematic, metro-map-style layouts for fitted spline route networks."""
...
class MetroLayout:
    ...
    def _extend_junction_stubs(self, paths: list[np.ndarray]) -> list[np.ndarray]:
        ...
        for route, sides in by_route.items():
            chain = self.model.route_chains_[route]
    ...
    def _set_residual_scale(self, result: Any | None) -> float:
        if result is None:
    ...
    def _standardized_residual(self, result: Any) -> np.ndarray:
    ...
    def _fit_residual_axes(self, result: Any | None) -> None:
    ...
    def transform_points(self, result: Any) -> np.ndarray:
        ...
        for route, path in enumerate(self.route_paths_):
            ...
            if not np.any(members):
                continue
            spline = self.model.routes_[route]
