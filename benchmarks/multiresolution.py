...
def benchmark(n, fit_coarse=False):
    rng = np.random.default_rng(0)
    t = rng.uniform(0, 2*np.pi, n)
    points = np.column_stack([np.cos(t), np.sin(t), rng.normal(0, .01, n)])
    start = perf_counter()
    levels = build_hierarchy(points, representative_method="approx_medoid")
    result = {'n': n, 'hierarchy_seconds': perf_counter()-start,
              'level_sizes': [len(level.points) for level in levels]}
    if fit_coarse:
        coarse = levels[-1].points
        if len(coarse) > 1000:
            result['coarse_fit'] = 'skipped: hierarchy remains above 1000 representatives'
        else:
            start = perf_counter()
            model = SkeletalEmbedding(use_multiresolution=False, standardize=False).fit(coarse)
            result.update(coarse_fit_seconds=perf_counter()-start,
                          topology_input_sizes=model.topology_input_sizes_,
                          mip_candidate_count=model.mip_candidate_count_)
    rss = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
    result['process_peak_rss_mb'] = rss / (1024**2 if sys.platform == 'darwin' else 1024)
    return result
...
if __name__ == '__main__':
    ...
    for n in args.sizes:
        ...
        print(json.dumps(benchmark(n, args.fit_coarse)), flush=True)
